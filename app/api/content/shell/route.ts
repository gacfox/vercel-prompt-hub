...
export async function POST(request: Request) {
  try {
    ...
    const result = await sql`
      ...
      VALUES (${user.userId}, 'shell', ${title.trim()}, ${description}, ${is_public})
      RETURNING id
    `;
    ...
    await sql`
      INSERT INTO vph_content_shell (content_id, shell_type, command)
      VALUES (${contentId}, ${safeShellType}, ${command})
    `;
    ...
    console.error("[vph] Shell publish error:", error);
    return NextResponse.json(
